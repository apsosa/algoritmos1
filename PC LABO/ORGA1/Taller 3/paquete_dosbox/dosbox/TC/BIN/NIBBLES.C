...
void far rutina_teclado()
{
	...
	// COMPLETAR RUTINA
	if(presionada){
		if(tecla_anterior = 0xE0){

			if(scancode == TECLA_LEFT){
...
int main(void)
{
	...
	orig_timer = get_isr(TIMER);
	orig_teclado = get_isr(TECLADO);
	set_isr(TECLADO,rutina_teclado);
	set_isr(TIMER,rutina_reloj);
	...
	set_isr(TECLADO,orig_teclado);
	set_isr(TIMER,orig_timer);
